...
- [x] One config module holds every Appendix A constant, including `AS_OF_DATE`. No other file hard-codes a threshold, and no code reads the system clock for "today".
- [x] Normalization keeps only individuals who are not deceased, and only gifts with `status = paid` and `gift_type != recurring_parent`. Unit tests cover each rule.
- [x] SIG1 returns 1,508 signals for the 1,530 individuals with a recent gift (Appendix B).
- [x] Tests cover a gift exactly 365 days before `AS_OF_DATE` (recent) and a stewardship interaction dated the same day as the gift (counts as subsequent).
- [x] Valerie Kaur (constituent 2669) appears as THANK with evidence "$25,000 gift on Feb 4, 2026" and "No stewardship interaction is recorded since the gift".
- [x] No text anywhere says a donor "was never thanked" (G4).
- [x] `GET /api/today` returns the signals as JSON, and the Today page renders action label and evidence for each.
- [x] Missing values render as "Not on file", never as blanks or guesses (G4).
- [x] A README explains how to run the backend, the frontend, and the tests.
